...
- [ ] **T8｜Demo 验活入口对齐（诊断 P0-3）**
  ...
  - 当前状态：README、自动检查、截图和兜底说明已齐备；当前投递版 PDF 的 Demo/GitHub 链接注释目标正确。仍等待本人填写三端真实验活结果，Codex 不代填。
...
- [ ] **T9｜路线甲——补齐受控测试证据**
  ...
  - 当前状态：本人已选择路线甲和公开脱敏 CSV；分析器、测试、数据契约与 manifest 对照校验已实现。当前投递版简历已核对，确认仍包含受控测试未验证数字及越界的“测试内采用率”命名；等待本人提供并批准真实 CSV、核对计算结果并同步简历措辞。
