...
export function RunDetailClient({ initialRun, user, adminNavigation }: { initialRun: EvaluationRunRecord; user: PublicUser; adminNavigation?: AdminNavigationFlags }) {
  ...
  const pairwiseDecisionCount = run.pairwiseDecisions
  ...
  return (
    <div className="min-h-screen">{adminNavigation ? <AdminWorkspaceHeader {...adminNavigation} /> : <AppHeader />}<main className="mx-auto w-full max-w-7xl px-4 py-7 pb-20 md:px-6">
      ...
      <header className="mt-5 flex flex-col gap-4 border-b border-[var(--color-line-strong)] pb-6 md:flex-row md:items-end md:justify-between"><div><p className="flex items-center gap-2 text-xs font-black text-[var(--color-accent)]"><Flask size={15} weight="bold" />{formatDataOrigin(run.dataOrigin)} · {formatExecutionMode(run.executionMode)}</p><h1 className="mt-3 text-3xl font-black tracking-[-0.04em]">{run.runName}</h1><p className="mt-2 text-xs text-[var(--color-muted)]">状态：{formatRunStatus(run.status)} · {run.caseCount} 个固定案例 · 当前身份：{user.displayName}（{formatUserRole(user.role)}）</p></div>{user.role === 'admin' && <button className="button-secondary" onClick={loadReport} type="button">查看升级报告</button>}</header>

      <section className="mt-6 grid overflow-hidden rounded-[14px] border border-[var(--color-ink)] bg-white sm:grid-cols-4">{[
        ['生成任务',`${generated}/${totalTasks}`],['候选结果',`${run.candidates?.length ?? Number((run as unknown as { candidateCount?: number }).candidateCount ?? 0)}/${run.caseCount * 6}`],['正式结果',`${run.formalResults?.length ?? Number((run as unknown as { selectedCount?: number }).selectedCount ?? 0)}/${run.caseCount * 2}`],['A/B 定案',`${pairwiseDecisionCount}/${run.caseCount}`]
      ...
      {report && reportSummary && (
        <section className="editorial-panel mt-6 overflow-hidden" aria-labelledby="evaluation-report-heading">
          ...
          <div className="p-5">
            ...
            <div className="mt-5 flex flex-wrap gap-2">{['evaluation_cases.csv','evaluation_generations.csv','human_evaluations.csv','pairwise_evaluations.csv','bad_cases.csv','evaluation_report.json','evaluation_report.md'].map((file) => <a className="button-secondary" href={`/api/evaluation/runs/${run.id}/export?file=${file}`} key={file}>{file}</a>)}</div>
