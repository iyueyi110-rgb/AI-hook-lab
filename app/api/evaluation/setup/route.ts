...
export async function POST(request: Request) {
  try {
    ...
    return isForm ? createEvaluationFormRedirect(request.url) : NextResponse.json({ ok: true, user: publicUser(session.user) });
  } catch (error) {
    const isForm = request.headers.get("content-type")?.includes("application/x-www-form-urlencoded") || request.headers.get("content-type")?.includes("multipart/form-data");
    return isForm
      ? createEvaluationFormRedirect(request.url, "setup_failed")
