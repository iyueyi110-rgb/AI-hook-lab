...
- 生成结果包含 Prompt 版本、模型和变体，便于回溯与 A/B 对照
...
看板数据来自服务端文件 `data/dashboard-events.json`。该文件由 `/api/dashboard/events` 写入，用于统计生成完成率、收藏率、采用率、复制率、平台适配满意度、平台分布、Prompt 版本分布、平台表现对比和 bad case 分布。运行数据已被 `.gitignore` 忽略，不会进入提交。
...
汇总脚本会读取 `eval/results/for-scoring.csv`，输出 `eval/results/summary.json`，包含平均点击欲望、可用率、Prompt 版本分布、平台表现和 Top bad case。
...
结构化 Prompt 将 `platform`、`topic`、`contentType`、`targetAudience`、`emotionTone`、`wordLimit` 映射到模型输出。Prompt 模板集中在 `lib/promptTemplates.ts`，当前基线版本为 `v1.0.0`，变体为 `baseline`。输出字段包含 Hook 文案、风格标签、四维评分、0-100 点击欲望评分、推荐理由和整体分析。推荐理由要求引用具体词句，避免“运用了悬念手法”这类空泛套话。
