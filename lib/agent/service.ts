...
function asResponse(run: StoredAgentRun, messages: Message[] = [], finalizedResponse?: GenerateResponse): CoachRunResponse {
  ...
  return {
    ...
    allowedCommands: getAllowedCommands(run.status),
    needsInput: run.status === "understanding" || run.status === "analyzing_image" || run.status === "awaiting_brief_confirmation" || run.status === "reviewing" || run.status === "awaiting_final_confirmation",
...
function applyMemoryToBrief(input: Record<string, unknown>, entries: Array<{ key: MemoryKey; value: string }>): Record<string, unknown> {
  ...
  for (const entry of entries) {
    ...
    if (entry.key === "avoid_badcase_tag") {
...
export function createCreativeCoachService(dependencies: CreativeCoachDependencies) {
  ...
  async function completeGeneration(ownerId: string, runId: string, request: CoachGenerationRequest, revision: number): Promise<CoachRunResponse> {
  ...
  return {
    ...
    async submitTurn(sessionToken: string | undefined, runId: string, expectedRevision: number, command: AgentCommand): Promise<CoachRunResponse> {
      ...
      const owner = await sessionFromToken(sessionToken, false);
      let decidedBriefPatch: Record<string, unknown> | undefined;
      if (command.type === "message" && dependencies.decideBriefPatch) {
        const preview = findOwnedRun(await repository.read(), runId, owner.session.id);
        assertExpectedRevision(preview, expectedRevision);
        if (preview.status === "understanding") {
          const currentBrief = preview.brief ?? preview.briefDraft ?? {};
          const current = normalizeBrief(currentBrief, preview.clarificationAttempts ?? 0);
          const missingField = current.kind === "complete" ? undefined : current.missing[0];
          if (missingField) {
            const directPatch = parseClarificationValue(missingField, command.text);
            const direct = normalizeBrief({ ...currentBrief, ...directPatch }, preview.clarificationAttempts ?? 0);
            const directMissing = direct.kind === "complete" ? undefined : direct.missing[0];
            if (direct.kind !== "complete" && directMissing === missingField) {
              try {
                const proposed = await dependencies.decideBriefPatch({ message: command.text, missingField, currentBrief });
                assertSafeBriefInput(proposed);
                decidedBriefPatch = proposed;
              } catch {
                // A clarification model failure is safely recoverable through the
                // deterministic single-field path, without storing model reasoning.
              }
            }
          }
        }
      ...
      const prepared = await repository.transaction((state) => {
        ...
        if (run.status === "understanding" && command.type === "message") {
          const current = normalizeBrief(run.brief ?? run.briefDraft ?? {}, run.clarificationAttempts ?? 0);
          const missingField = current.kind === "complete" ? undefined : current.missing[0];
          const source = { ...(run.brief ?? run.briefDraft ?? {}), ...(missingField ? decidedBriefPatch ?? parseClarificationValue(missingField, command.text) : {}) };
          run.briefDraft = source;
          const normalized = normalizeBrief(source, run.clarificationAttempts ?? 0);
          addedMessages.push({ id: id("message"), role: "user", content: command.text, createdAt: timestamp });
          if (normalized.kind === "complete") {
            ...
            addedMessages.push({ id: id("message"), role: "assistant", content: questionFor(normalized.missing[0] ?? "topic"), createdAt: timestamp });
          } else {
            run.requiresFormCompletion = true;
            addedMessages.push({ id: id("message"), role: "assistant", content: "Please complete the structured brief form.", createdAt: timestamp });
          }
        ...
        if (command.type === "retry") {
          ...
          return { kind: "generate" as const, request: { ...pending, brief: run.brief, sourceCandidate } satisfies CoachGenerationRequest, revision: run.revision };
        }
        ...
        if (command.type === "confirm_brief") {
          ...
          run.updatedAt = timestamp;
          return { kind: "generate" as const, request: { kind: "initial", count: 10, brief: run.brief } satisfies CoachGenerationRequest, revision: run.revision };
        }
      ...
      return prepared.kind === "response" ? prepared.response : completeGeneration(owner.session.id, runId, prepared.request, prepared.revision);
    },
    ...
    async uploadImage(sessionToken: string | undefined, runId: string, expectedRevision: number, file: File): Promise<CoachRunResponse> {
      ...
      const reserved = await repository.transaction((state) => {
        ...
        run.updatedAt = now().toISOString();
        return { revision: run.revision, callId, existingBrief: run.brief };
      });
      try {
        const analysis = await dependencies.analyzeImage(file);
